...
- `/column` — column articles. **Currently backed by `mockAllColumns` in `lib/mockData.ts`**, not Sanity. There is no `column` schema yet, though `Column` is typed in `types/index.ts`. If adding Sanity support for columns, add a schema + query and swap the mock import.
- `/contact` — 無料相談 form
...
Five document types, registered in `sanity/schemas/index.ts`:
...
- **ctaConfig** — singleton for primary/secondary CTA buttons. If this document doesn't exist, Hero and StickyCTA render nothing — see `SANITY_INIT.md`.

The hardcoded tag/problem option lists are duplicated between `case.ts`, `method.ts`, and the union types in `types/index.ts` (`CaseProblem`, `TechTag`). Keep all three in sync when adding values.

`Column` exists as a TS type but has no corresponding Sanity schema (see routing note above).
...
- `searchQuery` returns `{cases, methods, services}` in one request and is used by `/search`.
